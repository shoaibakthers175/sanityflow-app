...
class PDFService {
  /**
   * Generate an Executive QA Sanity Report PDF
   * @param {Object} session - Checklist session object with items and stats
   * @param {string} customSavePath - Optional custom output path
   * @returns {Promise<{success: boolean, filePath?: string, error?: string}>}
   */
  async generatePDF(session, customSavePath = null) {
    try {
      if (!session) {
        throw new Error('No checklist session data provided');
      }
      ...
      // ==========================================
      // 4. HIERARCHICAL CHECKLIST TABLE
      // ==========================================
      const tableData = [];
      ...
      items.forEach((item) => {
        ...
        const hasEvidence = item.screenshot_path ? 'Attached' : '-';
      ...
      // ==========================================
      const itemsWithScreenshots = items.filter(item => {
        if (!item.screenshot_path) return false;
        if (item.screenshot_path.startsWith('data:image')) return true;
        return fs.existsSync(item.screenshot_path);
      });
        ...
        itemsWithScreenshots.forEach((item, idx) => {
          ...
          try {
            ...
            if (item.screenshot_path.startsWith('data:image')) {
              imgDataUri = item.screenshot_path;
            } else if (fs.existsSync(item.screenshot_path)) {
              const imgBuffer = fs.readFileSync(item.screenshot_path);
              const ext = path.extname(item.screenshot_path).toLowerCase();
              if (ext === '.jpg' || ext === '.jpeg') format = 'JPEG';
              if (ext === '.webp') format = 'WEBP';
              const base64Data = imgBuffer.toString('base64');
              imgDataUri = `data:image/${format.toLowerCase()};base64,${base64Data}`;
            }

            if (imgDataUri) {
              const imgX = margin + 5;
              const imgYPos = imgY + (item.notes ? 15 : 12);
              const imgW = contentWidth - 10;
              const imgH = 56;
              doc.addImage(imgDataUri, format, imgX, imgYPos, imgW, imgH, undefined, 'FAST');
            }
          } catch (imgErr) {
            console.error('[PDFService] Error embedding image:', imgErr);
            doc.setFont('helvetica', 'normal');
            doc.setTextColor(...colorFail);
            doc.text(`[Screenshot attached: ${item.screenshot_path}]`, margin + 10, imgY + 25);
          }
      ...
      // Save PDF to disk
      let targetPath = customSavePath;
      if (!targetPath) {
        const safeProjectName = (session.project_name || 'SanityReport').replace(/[^a-zA-Z0-9_-]/g, '_');
        const defaultFilename = `${safeProjectName}_QA_Sanity_Report.pdf`;
        const defaultDir = app ? app.getPath('downloads') : path.join(__dirname, '../../');
        targetPath = path.join(defaultDir, defaultFilename);
      }

      const pdfOutput = doc.output('arraybuffer');
      fs.writeFileSync(targetPath, Buffer.from(pdfOutput));
      console.log('[PDFService] Generated and saved PDF to:', targetPath);
  ...
  /**
   * Prompt user with Save As dialog and generate PDF
   */
  async promptAndExportPDF(browserWindow, session) {
    const safeProjectName = (session.project_name || 'SanityReport').replace(/[^a-zA-Z0-9_-]/g, '_');
    const defaultFilename = `${safeProjectName}_QA_Sanity_Report.pdf`;

    const { canceled, filePath } = await dialog.showSaveDialog(browserWindow, {
      title: 'Export Executive QA Sanity PDF Report',
    ...
    const result = await this.generatePDF(session, filePath);
    return result;
  }
}
